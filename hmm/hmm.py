...
def norm_pdf_multi(x, mu, sigma):
  size = len(x)
  if size == len(mu) and (size, size) == sigma.shape:
    det = np.linalg.det(sigma)
    if det==0:
      raise NameError("The covariance matrix can't be signular")
...
class Model(object):
    ...
    def emit_prob(self, state, symbol):
        ...
        if state not in self._states:
            return 0
        x = symbol
        mu = self._emit_prob[state]["mean"]
        sigma = self._emit_prob[state]["cov"]
        return norm_pdf_multi(x, mu, sigma)
    ...
    def _forward(self, sequence):
        ...
        for state in self._states:
            ...
            #print self._emit_prob[state].logpdf(sequence[0])
