...
Pts1 = [[0, 0], [CLength / 2, 0], [CLength / 2, CWidth], [0, CWidth],
        [CLength, CWidth], [CLength, 0], [0, 0], [0, CWidth]]
Pts2 = [[0, 518], [0, 1006], [FreeLine, 1006], [FreeLine, 518]]
Pts3 = [[CLength, 518], [CLength, 1006], [
    CLength - FreeLine, 1006], [CLength - FreeLine, 518]]
Pts4 = [(HOOP, CWidth / 2), (FreeLine, CWidth / 2),
        (CLength - HOOP, CWidth / 2), (CLength - FreeLine, CWidth / 2)]
Pts5 = [[0, 579], [0, 945], [FreeLine, 945], [FreeLine, 579]]
Pts6 = [[CLength, 579], [CLength, 945], [
    CLength - FreeLine, 945], [CLength - FreeLine, 579]]
Pts7 = [(0, SPACE), (TSLine, SPACE),
        (0, CWidth - SPACE), (TSLine, CWidth - SPACE)]
Pts8 = [(CLength, SPACE), (CLength - TSLine, SPACE),
        (CLength, CWidth - SPACE), (CLength - TSLine, CWidth - SPACE)]
Pts9 = [(SideLine, 0), (SideLine, CWidth),
        (CLength - SideLine, 0), (CLength - SideLine, CWidth)]
Pts10 = [(FreeLine - RLine, CWidth / 2), (FreeLine + RLine, CWidth / 2), (CLength - FreeLine - RLine, CWidth / 2),
         (CLength - FreeLine + RLine, CWidth / 2), (HOOP + TLine,
                                                    CWidth / 2), (CLength - TLine - HOOP, CWidth / 2),
         (CLength / 2, CWidth / 2)]
...
Pts12 = [(CLength / 2, CWidth / 2)]

Pts1 = (np.array(Pts1, np.int32) +
        np.array([marginX, marginY])).reshape((-1, 1, 2))
Pts2 = (np.array(Pts2, np.int32) +
        np.array([marginX, marginY])).reshape((-1, 1, 2))
Pts3 = (np.array(Pts3, np.int32) +
        np.array([marginX, marginY])).reshape((-1, 1, 2))
Pts4 = np.array(Pts4, np.int32) + np.array([marginX, marginY])
Pts5 = (np.array(Pts5, np.int32) +
        np.array([marginX, marginY])).reshape((-1, 1, 2))
...
Pts12 = np.array(Pts12, np.int32) + np.array([marginX, marginY])

Bound = [(0, 0), (CLength, 0), (0, CWidth), (CLength, CWidth)]
Bound = (np.array(Bound, np.float32) +
         np.array([marginX, marginY])).reshape(-1, 1, 2)
Bound /= 2
...
def visM():
    ...
    # + np.array([[0,0,100], [0,0,200], [0,0,1]])
    return np.dot(np.diag([1.23, 1., 1.]), M)
...
def template():
    ...
    tpl = cv2.ellipse(
        tpl, tuple(Pts4[0]), (TLine, TLine), 270, 21, 159, 255, 5)
    tpl = cv2.ellipse(
        tpl, tuple(Pts4[0]), (RBlock, RBlock), 270, 0, 180, 255, 5)
    tpl = cv2.circle(tpl, tuple(Pts4[1]), RLine, 255, 5)
    tpl = cv2.ellipse(tpl, tuple(Pts4[2]), (TLine, TLine), 90, 21, 159, 255, 5)
    tpl = cv2.ellipse(
        tpl, tuple(Pts4[2]), (RBlock, RBlock), 90, 0, 180, 255, 5)
    tpl = cv2.circle(tpl, tuple(Pts4[3]), RLine, 255, 5)
    tpl = cv2.circle(tpl, tuple(Pts12[0]), RLine, 255, 5)
    ...
    tpl = cv2.line(
        tpl, tuple(Pts9[0]), tuple(Pts9[0] + np.array([0, SPACE])), 255, 5)
    tpl = cv2.line(
        tpl, tuple(Pts9[1]), tuple(Pts9[1] - np.array([0, SPACE])), 255, 5)
    tpl = cv2.line(
        tpl, tuple(Pts9[2]), tuple(Pts9[2] + np.array([0, SPACE])), 255, 5)
    tpl = cv2.line(
        tpl, tuple(Pts9[3]), tuple(Pts9[3] - np.array([0, SPACE])), 255, 5)
    #cv2.polylines(tpl,[Pts2], True, 255, 20)
    tpl = cv2.resize(tpl, None, fx=.4, fy=.4)
    return tpl
